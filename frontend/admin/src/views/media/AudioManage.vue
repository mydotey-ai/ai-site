  ...
  <div class="audio-manage">
    <div class="header">
      ...
      <n-space>
        ...
        <n-button type="primary" @click="openModal()">添加外链音频</n-button>
      </n-space>
    </div>
    ...
    <!-- 添加/编辑弹窗 -->
    <n-modal v-model:show="showModal" preset="card" :title="editId ? '编辑音频' : '添加音频'" style="width: 600px">
      <n-form ref="formRef" :model="form" :rules="rules" label-placement="left" label-width="80">
        <n-form-item label="标题" path="title">
          <n-input v-model:value="form.title" placeholder="请输入音频标题" />
        </n-form-item>
        ...
        <n-form-item v-if="form.type === 'EXTERNAL'" label="音频ID" path="audioId">
          <n-input v-model:value="form.audioId" placeholder="外链音频ID" />
        </n-form-item>
    ...
    <!-- 上传弹窗 -->
    <n-modal v-model:show="showUploadModal" preset="card" title="上传音频" style="width: 500px">
      <n-upload :custom-request="handleUpload" accept="audio/*">
        <n-upload-dragger>
...
import { audioApi } from '@/api/media'
import type { Audio, AudioRequest } from '@/types'
...
const form = reactive<AudioRequest>({
  title: '',
  type: 'EXTERNAL',
  platform: 'NETEASE',
  audioId: '',
...
const columns = [
  ...
  { title: '分类', key: 'category', width: 100 },
  {
    ...
    key: 'isPublic',
    width: 80,
    render: (row: Audio) => h(NTag, { type: row.isPublic ? 'success' : 'default' }, () => row.isPublic ? '公开' : '私有')
  },
  {
    title: '操作',
    key: 'actions',
    width: 150,
    render: (row: Audio) => h(NSpace, {}, () => [
...
async function loadData() {
  loading.value = true
  try {
    const res = await audioApi.getList({ page: pagination.page, size: pagination.pageSize })
...
function openModal(audio?: Audio) {
  if (audio) {
    editId.value = audio.id
    Object.assign(form, {
      title: audio.title,
      type: audio.type,
    ...
    editId.value = null
    Object.assign(form, { title: '', type: 'EXTERNAL', platform: 'NETEASE', audioId: '', description: '', category: '', isPublic: 1 })
  }
...
async function handleSave() {
  ...
  try {
    if (editId.value) {
      await audioApi.update(editId.value, form)
      message.success('更新成功')
    } else {
      await audioApi.create(form)
